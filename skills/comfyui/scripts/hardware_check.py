...
Improvements over v1:
  ...
  - AMD on Windows: detects Radeon GPUs via PowerShell WMI when rocm-smi is unavailable
...
MIN_FREE_DISK_GB = 25  # ComfyUI core ~5 GB + one model ~5–24 GB
    ...
    "nvidia": "--nvidia",
    "amd": "--amd",
    "amd-windows": "--skip-torch-or-directml",
...
def detect_windows_amd() -> dict | None:
    """Detect AMD GPUs on Windows via PowerShell (no rocm-smi available there).

    This catches AMD/Radeon GPUs that nvidia-smi won't see and rocm-smi
    can't query on Windows. Returns the first AMD GPU found with VRAM.
    """
    ...
    out = _run([
        ...
        "Where-Object { $_.Name -match 'AMD|Radeon|Radeon\\s*RX|Radeon\\s*Pro' } | "
    ...
    for line in out.splitlines():
        ...
        if "AdapterRAM" in line and ":" in line:
            try:
                ...
                pass
    if name:
        return {"vendor": "amd", "name": name, "vram_gb": vram_gb, "driver": "directml"}
    return None
...
def classify(gpu: dict | None, ram_gb: float, free_disk_gb: float, *, wsl: bool, rosetta: bool) -> tuple[str, str, list[str]]:
    ...
    # Windows AMD -> DirectML backend, add install note
    if gpu.get("vendor") == "amd" and gpu.get("driver") == "directml":
        notes.append(
            f"AMD GPU on Windows detected ({name}). Use DirectML backend: "
            "`comfy --skip-prompt install --skip-torch-or-directml`, "
            "then `pip install torch-directml`, then launch with `comfy launch -- --directml`."
        )
    ...
    if vram < OK_VRAM_GB:
        ...
        install_path = "amd-windows" if gpu.get("driver") == "directml" else gpu["vendor"]
        return "marginal", install_path, notes
    if vram < GREAT_VRAM_GB:
        notes.append(f"{name} ({vram} GB VRAM) — SDXL comfortable, Flux possible with optimizations.")
        install_path = "amd-windows" if gpu.get("driver") == "directml" else gpu["vendor"]
        return "ok", install_path, notes
