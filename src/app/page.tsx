...
import { 
  ...
  Trash2, Wand2, Activity, Play, Pause, Palette, ImageIcon, Download, Video
} from "lucide-react";
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <header className="flex justify-between items-center mb-12">
          <div>
            ...
            <p className={`mt-1 transition-colors duration-500 ${s.subText}`}>Cinematic Local Story Studio</p>
          </div>
          <AnimatePresence>
            {showNav && (
              <motion.div 
            ...
            {isLoading ? (
              ...
              <div className={`text-center py-24 rounded-3xl border-2 border-dashed transition-colors duration-500 ${s.emptyState}`}>
                ...
                <h3 className="text-xl font-medium text-zinc-300">
                  {!Array.isArray(stories) ? "Error loading library" : "No stories yet"}
                </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {stories.map((story: any) => (
                  <Link 
                    ...
                    className={`group relative h-80 rounded-2xl overflow-hidden border transition-all duration-500 ${s.card}`}
                  >
                  ...
                  <button
                    ...
                    className={`absolute top-4 left-4 z-30 p-3 rounded-full backdrop-blur-md border border-white/10 transition-all ${
                  ...
                  {playingStoryId === story.id && (
                    <div className="absolute top-4 left-16 z-30 flex items-center gap-2 px-3 py-1.5 bg-amber-400/10 border border-amber-400/20 rounded-full backdrop-blur-md">
                  ...
                  <div className="absolute top-4 right-4 z-30 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {story.videoPath && (
                      <a
                        ...
                        className="p-2 bg-black/40 hover:bg-emerald-500/80 text-white/70 hover:text-white rounded-full backdrop-blur-md border border-white/10 transition-all"
                    ...
                    {story.fullAudioPath && (
                      <a
                        href={story.fullAudioPath}
                        download={`${story.title}.mp3`}
                        onClick={(e) => e.stopPropagation()}
                        className="p-2 bg-black/40 hover:bg-blue-500/80 text-white/70 hover:text-white rounded-full backdrop-blur-md border border-white/10 transition-all"
                        title="Download Full Audiobook"
                      >
                        <Download size={18} />
                      </a>
                    )}
                    <button
                      ...
                      className="p-2 bg-black/40 hover:bg-red-500/80 text-white/70 hover:text-white rounded-full backdrop-blur-md border border-white/10 transition-all"
                      title="Delete Story"
                    >
                      <Trash2 size={18} />
                    </button>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 z-20">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="inline-block px-2 py-0.5 bg-zinc-800 text-zinc-400 text-[10px] uppercase tracking-wider rounded">
                        {story.genre}
                      </span>
                      {story.series && (
                        <span className="inline-block px-2 py-0.5 bg-amber-400/10 text-amber-400 border border-amber-400/20 text-[10px] uppercase tracking-wider rounded font-bold">
                          {story.series.title}
                        </span>
                      )}
                    </div>
                    <h3 className="text-xl font-bold text-white group-hover:text-zinc-200 transition-colors">{story.title}</h3>
                    <div className="flex items-center justify-between mt-3 text-sm text-zinc-500">
                      ...
                      <span className={`capitalize px-2 py-0.5 rounded-full text-[11px] border ${
