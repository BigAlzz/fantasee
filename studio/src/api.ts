...
export const api = {
  ...
  extendStory: (storyId: string, input: { scenes?: number; duration_minutes?: number; prompt?: string; images_per_scene?: number; voice?: string; tone?: string }) => request<{ status: string; new_scenes_added: number; total_scenes: number; titles: string[]; errors: string[] }>(`/api/stories/${storyId}/extend`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(input) }),
