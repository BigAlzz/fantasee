...
import { BookOpen, ChevronLeft, Image, Pencil, Play, Plus, RefreshCw, Star, Trash2, Users, Wrench, X } from "lucide-react";
...
export function StoryDetails({ story, onBack, onOpenEditor, onPlay, onDeleted }: StoryDetailsProps) {
  ...
  const [extensionMode, setExtensionMode] = useState<"scenes" | "duration">("scenes");
  const [extensionScenes, setExtensionScenes] = useState(3);
  ...
  const extendStory = async () => {
    ...
    setExtensionMessage("Preparing the continuation from the current story canon...");
    try {
      const result = await api.extendStory(story.id, {
        ...(extensionMode === "scenes" ? { scenes: extensionScenes } : { duration_minutes: extensionMinutes }),
        prompt: extensionPrompt.trim(),
      });
      setDetail(await api.story(story.id));
      setExtensionMessage(`${result.new_scenes_added} scene${result.new_scenes_added === 1 ? "" : "s"} added. ${result.errors.length ? `${result.errors.length} scene${result.errors.length === 1 ? "" : "s"} needs attention.` : "The continuation is saved to the story."}`);
      setExtensionOpen(false);
    } catch (reason) {
      setExtensionMessage(reason instanceof Error ? reason.message : "The story extension could not be generated.");
    } finally {
  ...
  return <div className="story-details-page">
    ...
    <section className="story-details-hero"><div className="story-details-cover">{cover ? <img src={cover} alt=""/> : <Image size={38}/>}</div><div className="story-details-hero-copy"><span className="eyebrow-label">Origin story</span><h1>{source.title}</h1><p>{source.description || "No synopsis has been recorded for this story yet."}</p><div className="story-details-actions"><button type="button" className="create" onClick={onOpenEditor}><Pencil size={15}/> Edit scenes and narration</button><button type="button" className="outline-button extension-trigger" disabled={extensionBusy || operationBusy || directionBusy} onClick={() => { setExtensionOpen((current) => !current); setExtensionMessage(undefined); }}><Plus size={15}/> {extensionOpen ? "Close extension" : "Extend story"}</button><button type="button" className="outline-button" onClick={() => setEditingDirection((current) => !current)}><BookOpen size={15}/> {editingDirection ? "Close story direction" : "Edit story direction"}</button><button type="button" className="outline-button" disabled={operationBusy || directionBusy} onClick={() => void repairStory()}><Wrench size={15}/> Repair missing assets</button><button type="button" className="outline-button" disabled={operationBusy || directionBusy} onClick={() => void rebuildStory()}><RefreshCw size={15}/> Re-run full generation</button><button type="button" className="outline-button" disabled={operationBusy || directionBusy} onClick={() => void retryCardArt()}><Image size={15}/> Retry card art</button><button type="button" className="outline-button" disabled={!source.completion?.full_video_ok} onClick={onPlay}><Play size={15}/> Play release</button><button type="button" className="danger" disabled={operationBusy || directionBusy} onClick={() => void deleteStory()}><Trash2 size={15}/> Delete story and assets</button></div>{operationMessage && <p className="story-details-operation-message">{operationMessage}</p>}</div><div className="story-details-rating"><span><Star size={16}/>{rating ? `${rating}/10` : "Unrated"}</span><small>Creative review</small></div></section>
    {extensionOpen && <section className="story-details-panel story-extension-panel"><div className="story-details-heading"><div><span className="eyebrow-label">Continuation director</span><h2>Keep the story moving.</h2></div><small>New scenes inherit the current cast, style, voice, and canon.</small></div><div className="extension-mode-tabs"><button type="button" className={extensionMode === "scenes" ? "selected" : ""} onClick={() => setExtensionMode("scenes")}>Add scenes</button><button type="button" className={extensionMode === "duration" ? "selected" : ""} onClick={() => setExtensionMode("duration")}>Add runtime</button></div><div className="story-extension-fields">{extensionMode === "scenes" ? <label><span>Scenes to add</span><input type="number" min="1" max="50" value={extensionScenes} onChange={(event) => setExtensionScenes(Math.max(1, Math.min(50, Number(event.target.value) || 1)))}/><small>Each scene receives its own artwork, narration, subtitles, and timeline entry.</small></label> : <label><span>Minutes to add</span><input type="number" min="0.5" max="240" step="0.5" value={extensionMinutes} onChange={(event) => setExtensionMinutes(Math.max(0.5, Math.min(240, Number(event.target.value) || 0.5)))}/><small>The engine estimates scene count from this story's average scene duration.</small></label>}<label className="extension-prompt"><span>Continuation instruction</span><textarea value={extensionPrompt} onChange={(event) => setExtensionPrompt(event.target.value)} placeholder="Bring the survivors to the final gate; the commander sacrifices himself and the story concludes with the city choosing peace."/><small>Describe the turn, reveal, conclusion, or character fate you want. This is passed to the writer as a direction, not treated as a guarantee until the generated scenes are reviewed.</small></label></div><div className="story-extension-actions"><button type="button" className="create" disabled={extensionBusy} onClick={() => void extendStory()}><Plus size={15}/> {extensionBusy ? "Generating continuation..." : "Generate continuation"}</button>{extensionMessage && <span className="story-details-operation-message">{extensionMessage}</span>}</div></section>}
