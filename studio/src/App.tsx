...
import { api, type ComfyWorker, type GenerateInput, type MigrationReadiness, type ProductionEvent, type ProductionJob, type ProductionRelease, type ProductionRun, type Scene, type SeedSuggestion, type SemanticShot, type ShotAsset, type Story, type StoryDetail, type StudioSettings, type SubtitleCue, type TimelineShot, type Worker } from "./api";
...
function StudioDesk({ view, stories, runs, workers, selectedStory, jobs, busy, onSpawn, onRefresh, onSelectRun, onPreviewRelease }: { view: string; stories: Story[]; runs: ProductionRun[]; workers: Array<Worker | ComfyWorker>; selectedStory?: Story; jobs: ProductionJob[]; busy: boolean; onSpawn: (kind: "cpu" | "gpu") => void; onRefresh: () => void; onSelectRun: (id: string) => void; onPreviewRelease: (release: ProductionRelease) => void }) {
  ...
  return <section className="desk-panel metal-panel"><div className="eyebrow"><span>{title}</span><span>{view === "Productions" ? `${runs.length} durable runs` : view === "Workers" ? `${workers.length} active signals` : "Operator view"}</span></div>
    ...
    {view === "Assets" && <><h1>Approved inputs only.</h1><p className="desk-intro">Candidates remain reversible until explicitly approved. Current story evidence is the release gate.</p>{migrationLoading && <div className="migration-loading"><span className="led amber"/>Scanning every story for migration and rollback evidence...</div>}{migration && <MigrationSummary readiness={migration} />}{selectedStory ? <><div className="asset-ledger">{completionRows(selectedStory).map(([, label, complete, detail]) => <div className="asset-row" key={label}><span className={`led ${complete ? "green" : "amber"}`}/><strong>{label}</strong><small>{detail}</small></div>)}</div><ReleaseLedger releases={releases} onPreview={onPreviewRelease} /></> : <p className="ledger-empty">Select a story to inspect its assets.</p>}</>}
