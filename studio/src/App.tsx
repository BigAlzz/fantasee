...
function StoryEditor({ story, sceneIndex, busy, onClose, onSelectScene, onAction }: { story?: StoryDetail; sceneIndex: number; busy: boolean; onClose: () => void; onSelectScene: (index: number) => void; onAction: (action: () => Promise<unknown>, message: string) => void }) {
  ...
  useEffect(() => {
    if (!story) { setShots([]); setSceneDraft(undefined); setSelectedShot(undefined); setShotAssets([]); setShotRevisions([]); setTimelineStatus(undefined); setTimelineShots([]); return; }
    setSceneDraft(story.scenes[sceneIndex]);
    setSelectedShot(undefined); setShotAssets([]); setShotRevisions([]); setTimelineStatus(undefined); setTimelineShots([]); setShotLocked(false); setDragShotId(undefined);
