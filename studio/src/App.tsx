...
import { api, type ComfyWorker, type GenerateInput, type ProductionEvent, type ProductionJob, type ProductionRun, type Scene, type SeedSuggestion, type SemanticShot, type ShotAsset, type Story, type StoryDetail, type StudioSettings, type SubtitleCue, type TimelineShot, type Worker } from "./api";
...
function StudioDesk({ view, stories, runs, workers, selectedStory, jobs, busy, onSpawn, onRefresh, onSelectRun }: { view: string; stories: Story[]; runs: ProductionRun[]; workers: Array<Worker | ComfyWorker>; selectedStory?: Story; jobs: ProductionJob[]; busy: boolean; onSpawn: (kind: "cpu" | "gpu") => void; onRefresh: () => void; onSelectRun: (id: string) => void }) {
  ...
  return <section className="desk-panel metal-panel"><div className="eyebrow"><span>{title}</span><span>{view === "Productions" ? `${runs.length} durable runs` : view === "Workers" ? `${workers.length} active signals` : "Operator view"}</span></div>
    ...
    {view === "Settings" && <><h1>Keep the chain honest.</h1><p className="desk-intro">These controls are persisted locally and applied to new production work. Existing approved media is never silently rewritten.</p>{settings ? <div className="settings-form"><label>Voice<select value={settings.tts_voice_preset} onChange={(event) => setSettings({ ...settings, tts_voice_preset: event.target.value })}><option>Dean</option><option>Milo</option><option>Mia</option><option>Chloe</option></select></label><label>Speed<input type="number" min="0.5" max="3" step="0.05" value={settings.tts_speed} onChange={(event) => setSettings({ ...settings, tts_speed: Number(event.target.value) })}/></label><label>Default style<input value={settings.default_style} onChange={(event) => setSettings({ ...settings, default_style: event.target.value })}/></label><label>Default tone<input value={settings.default_tone} onChange={(event) => setSettings({ ...settings, default_tone: event.target.value })}/></label><label className="wide">ComfyUI workers<input value={settings.comfyui_urls} onChange={(event) => setSettings({ ...settings, comfyui_urls: event.target.value })}/></label><label className="wide">Plex destination<input value={settings.plex_destination} onChange={(event) => setSettings({ ...settings, plex_destination: event.target.value })}/></label><div className="settings-actions"><button className="outline-button" disabled={settingsBusy} onClick={() => void api.settings().then(setSettings)}>Reload</button><button className="create" disabled={settingsBusy} onClick={() => void saveSettings()}>Save settings</button></div></div> : <p className="ledger-empty">Loading validated settings...</p>}</>}
  </section>;
}
