...
export function LibraryCatalog({ stories, selectedStoryId, onOpenStory }: CatalogProps) {
  ...
  return <div className="library-catalog">
    ...
    {sorted.length ? mode === "cards" ? <div className="story-card-grid">{sorted.map((story) => { const state = storyState(story); const cover = storyCover(story); return <button type="button" className={`story-card ${story.id === selectedStoryId ? "selected" : ""}`} key={story.id} onClick={() => onOpenStory(story)}><div className="story-card-art">{cover ? <img src={cover} alt=""/> : <Image size={28}/>}<span className={`story-card-state ${state.tone}`}>{state.label}</span></div><div className="story-card-copy"><h3>{story.title}</h3><p>{story.description || "A production waiting for its first editorial note."}</p><div><span>{story.scene_count || 0} scenes</span><span>{dateLabel(story)}</span></div></div><ChevronRight size={17}/></button>; })}</div> : <div className="story-catalog-list">{sorted.map((story) => { const state = storyState(story); const cover = storyCover(story); return <button type="button" className={`story-catalog-row ${story.id === selectedStoryId ? "selected" : ""}`} key={story.id} onClick={() => onOpenStory(story)}><span className="story-catalog-thumb">{cover ? <img src={cover} alt=""/> : <Image size={15}/>}</span><span className="story-catalog-title"><strong>{story.title}</strong><small>{story.description || "No synopsis recorded."}</small></span><span className={`story-card-state ${state.tone}`}>{state.label}</span><span>{story.scene_count || 0} scenes</span><span>{dateLabel(story)}</span><ChevronRight size={17}/></button>; })}</div> : <div className="library-catalog-empty"><Image size={25}/><div><strong>Your story collection is ready for its first world.</strong><small>Create a story to see its artwork, canon, and production evidence here.</small></div></div>}
  </div>;
}
